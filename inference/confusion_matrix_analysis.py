...
def load_detections_csv(detections_path: str) -> List[Dict]:
    """
...
def load_labels_csv(labels_path: str) -> List[Dict]:
    """
...
def filter_by_filename(detections: List[Dict], labels: List[Dict]) -> tuple[List[Dict], List[Dict]]:
    """
    ...
    detection_files = set(normalize_filename(d['filename']) for d in detections)
    label_files = set(normalize_filename(l['filename']) for l in labels)
    ...
    filtered_detections = [d for d in detections if normalize_filename(d['filename']) in common_files]
    filtered_labels = [l for l in labels if normalize_filename(l['filename']) in common_files]
...
def get_species_list(detections: List[Dict], labels: List[Dict]) -> List[str]:
    """
...
def print_statistics(detections: List[Dict], labels: List[Dict], species_list: List[str]):
    """
...
Example:
    ...
    parser.add_argument(
        ...
        help='Output directory for results (default: inference/confusion_matrix_analysis)'
    )
    ...
    # Filter to common files
    detections, labels = filter_by_filename(detections, labels)
    
    if not detections:
        print("\nError: No detections to analyze!")
        return 1
    
    if not labels:
        print("\nError: No labels to analyze!")
        return 1
    
    # Get species list
    species_list = get_species_list(detections, labels)
    print(f"\nFound {len(species_list)} unique species: {', '.join(species_list)}")
    
    # Print statistics
    print_statistics(detections, labels, species_list)
    ...
    print_confusion_matrix(confusion_matrix, species_list, include_background=not args.no_background)
